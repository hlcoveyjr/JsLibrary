 ...
 * Gets the 3x3 box index of a 9x9 board from the provided 0-80 cell index (i)
 * @param {any} i
 *****************************************************************************/
let getBox = (i) => (parseInt(getRow(i) / 3) * 3) + parseInt(getColumn(i) / 3);
/******************************************************************************
 * Helper Method: getShape
 * Gets the shape coordinates index of a 9x9 board from the provided 0-80 cell 
 * index (i) where the shape might be different than the default 3x3 box.
 * @param {any} grid
 * @param {any} position
 *****************************************************************************/
function getShape(grid, position) {
    ...
    for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
            if (position == boxCoords[r][c]) {
                iArray.push(grid[r][c]);
            }
        }
    }
    return iArray;
...
class Board {
    ...
    print() {
        console.log(printGrid(this.board, this.id, "_"));
        /**********OLD WAY****************************
...
class Sudoku {
    ...
    constructor(id, puzzle, target, renderStyle) {
        ...
        this.given = new Board("Given", emptyBoard);
...
class SudokuCell {
    ...
    report() {
        ...
        let box = getShape(puzzle.board, this.id);
